...
import { Avatar, Box, Button, Card, CardActions, CardContent, Divider, IconButton, List, ListItem, ListItemAvatar, ListItemText, TextField, Tooltip, Typography } from "@mui/material";
import { addCart, decrementCount, incrementCount } from "../../redux/features/cart/cartSlice";
...
const Cart = () => {
    ...
    const CartCounter = ({ count, id }) => {
        return (
            <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", width: 20, alignContent: "center" }}>
                <button onClick={() => dispatch(decrementCount(id))} style={{ fontSize: 20, marginBottom: 4, border: "none", backgroundColor: "transparent", padding: 2, cursor: "pointer" }}>-</button>
                <span style={{ marginLeft: 4, marginRight: 4, backgroundColor: "blue", color: "white", borderRadius: 5, padding: 3 }}>{count}</span>
                <button onClick={() => dispatch(incrementCount(id))} style={{ fontSize: 20, border: "none", backgroundColor: "transparent", padding: 2, cursor: "pointer" }}>+</button>
            </Box>
        )
    }
    const listCart = () => {
